...
void Evaluate(std::string jsonFileName, double &force) {
  ...
  int iter = int(ceil((lato - (delta / 2)) /
                      delta)); // Replacement for "for (double i = delta / 2; i
                               // < lato; i = i + delta)"
...
#pragma omp parallel for schedule(guided, 16) // Same
    ...
    Epetra_SerialDenseMatrix xv0t, yv0t, xvft, yvft, pft,
        x0temp; // Temporary variables for warmup
    if (flagwarm == 1 && k > 1) {
