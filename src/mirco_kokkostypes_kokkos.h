...
// This file defines some commonly used Kokkos type aliases. More aliases, as well as Kokkos-related macros, utilities, etc., can be added as needed
...
// Note: LayoutLeft is column major and is necessary for KokkosLapack::gesv() in MIRCO::NonLinearSolver::solve(). We have to specify it for views of any rank, even if it is physically meaningless for rank < 2.
using ViewVector_h = Kokkos::View<double*, Kokkos::LayoutLeft, Device_Host_t>;
using ViewMatrix_h =
    Kokkos::View<double**, Kokkos::LayoutLeft, Device_Host_t>;
