    ...
    ViewVector_d& p_d, int& activeSetSize, double nnlstol, int maxiter)
{
  ...
  while (1)
  {
    ...
    while (2)
    {
      ...
      {
        Kokkos::parallel_for(
            "", Kokkos::RangePolicy<ExecSpace_Default_t>(0, 1), KOKKOS_LAMBDA(const int i) {
              b0s_compact_d(0) =
                  b0_d(activeInactiveSet(0)) / matrix(activeInactiveSet(0), activeInactiveSet(0));
            });
      }
        ...
        Kokkos::parallel_for(
            "assemble reduced system", activeSetSize, KOKKOS_LAMBDA(const int i) {
              p_d(activeInactiveSet(i)) =
                  p_d(activeInactiveSet(i)) +
                  minloc_alpha_i.val * (b0s_compact_d(i) - p_d(activeInactiveSet(i)));
            });
