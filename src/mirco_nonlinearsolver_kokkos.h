...
namespace MIRCO
{
  namespace NonLinearSolver
  {
    ...
    void solve(const ViewMatrix_d matrix, const ViewVector_d b0_d, ViewVector_d& p_d,
        int& activeSetSize, double nnlstol = 1.0e-08, int maxiter = 20000 /*10000*/);

    /**
     * @brief Compact and sort a ViewVectord_d w.r.t. a secondary ViewVectori_d. For example, p_d is
     * in the order of the original positional index, but some of those are 0 because they are
     * inactive; use activeSet to
     *
     * @param[in] matrix Influence coefficient matrix (Discrete version of Green Function)
     * @param[in] b0 Indentation value of the half space at the predicted points of contact.
    ...
    // # _h for now; we will see whether we can just keep on device more
    // [[nodiscard]]
    // ViewVector_d compactAndSort(ViewVector_d& p_d, ViewVectorInt_d& activeInactiveSet, bool
    // returnWRef = true, double nnlstol = 1.0e-08, int maxiter = 1000);
  };  // namespace NonLinearSolver
}  // namespace MIRCO
