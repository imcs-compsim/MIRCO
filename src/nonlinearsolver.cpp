...
void NonLinearSolver::NonlinearSolve(Epetra_SerialDenseMatrix &matrix,
...
#pragma omp parallel for schedule(static, 16) // Always same workload -> static
  ...
  while (aux1 == true) {
    // [wi,i]=min(w);
    // This is slightly slower than the optimal one. So far at least. Should
    // have a bit better scaling.
    // @{
    for (int i = 0; i < w.M(); i++) {
