  ...
  for (int i = 0; i < resolution; i++) {
    ...
    for (int j = d; j < N - d + 1; j = j + D) {
      for (int k = d; k < N - d + 1; k = k + D) {
        z(j, k) = (z(j + d, k + d) + z(j + d, k - d) + z(j - d, k + d) +
                   z(j - d, k - d)) /
                      4 +
    ...
    for (int j = d; j < N - d + 1; j = j + D) {
      z(j, 0) = (z(j + d, 0) + z(j - d, 0) + z(j, d)) / 3 +
                alpha * distribution(generate);
      z(j, N) = (z(j + d, N) + z(j - d, N) + z(j, N - d)) / 3 +
                alpha * distribution(generate);
      z(0, j) = (z(0, j + d) + z(0, j - d) + z(d, j)) / 3 +
                alpha * distribution(generate);
      z(N, j) = (z(N, j + d) + z(N, j - d) + z(N - d, j)) / 3 +
                alpha * distribution(generate);
    }
